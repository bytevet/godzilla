...
| TRUST-11 | med | ✅ | **Real-world CVE benchmark harness** — `test/cvebench` (opt-in `GODZILLA_CVE_BENCH=1`): a fix-diff-verified CVE manifest + a scan/score test reporting recall alongside the corpus F1. Regression guard for the COV-11/13 breadth gaps. Manifest refreshed to 7 recent (2024-2025) CVEs across Python/Ruby/JS (SSRF/SQLi/code-inj/cmd-inj/path-traversal); recall **3/7** after gap-closing work — langflow (FastAPI body source + ast propagators), motioneye (Tornado source + cross-module subprocess flow), decidim (Ruby module/class-method lowering + `Arel.sql` sink). The 4 remaining need deeper advances tracked here: mlflow (Python request-attribute alias `a = request.args; a.get()` → source), gradio (framework-internal pipeline), webpack (cross-module default-export + memoized-call resolution), pyload (thread-boundary + instance-field-persistence heap flow). |
