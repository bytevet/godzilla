...
| ENG-13 | med | ✅ | **Higher-order-callback taint + thread/async dispatch.** Both reduce to one primitive: the indirect call through a function VALUE — the already-legal CALL shape `{Callee:"", Value: FuncName\|RegName, IsInvoke:false}` (Go already emits it for `cb(x)`; the engine previously dropped it at `byKey[""]==nil`). **No gIR/proto change.** Engine (`interproc.go`): a `funcVal`/`paramFuncVal` points-to summary channel (cloned from the `reqEffects` provenance channel — a callback passed into a helper is tracked across the call boundary), a `targetsOf` resolver (a `FuncName` → `byKey`; a `RegName` → the points-to set), and a resolution branch that flows args into the target's front params and pulls its return taint back. Binds **only when the resolved target set is a singleton** — the `untyped_dispatch` unambiguous-only discipline verbatim — so a context-insensitive helper called with several callbacks accumulates a union and binds nothing (FP-safe: an ambiguous or unresolvable value is a false negative, never a false positive). Engine reads only structural IR (operand `Value` kind, `byKey`, taint maps) — **zero** language/library strings. Python frontend (`lower.go`): a local function used as a value resolves to its canonical `FuncName`; a param-held callee `fn(x)` lowers to the indirect shape; and `emitDeferredDispatch` rewrites `threading.Thread(target=…, args=(…))` / `Process` / `Executor.submit` / `run_in_executor` into a synthesized deferred call (library knowledge stays in the frontend). Verified: higher-order in **Python and Go** (Go with **no frontend change** — the engine channel alone resolves its indirect calls), thread dispatch, and the multi-callback singleton-gate suppression, each with vuln + safe corpus twins. An **adversarial review pass** then hardened four FP/correctness holes the corpus didn't cover: (1) a `Thread(target=self.run, args=(x,))` bound method now forwards the receiver as param 0 (else `x` bound to the `self` slot — a benign-field FP + real-arg FN); (2) an **opaque-callback channel** (`paramFuncOpaque`) marks a callback slot incomplete when any site passed an unresolvable value, so a resolvable singleton at one site can no longer be paired with another site's taint (cross-context FP); (3) the dispatch recognizer is scoped to `threading.Thread`/`multiprocessing.Process` with a resolved-FuncName target (`Executor.submit`/`run_in_executor` dropped pending executor-type tracking) so a same-named method is never mistaken for a dispatch; (4) forwarded args are lowered exactly once (the construct is fully consumed, returning an opaque handle) to avoid duplicate findings. A first hardening pass regressed `Scan_GoWithDeps` (+51% time/+38% mem) because the points-to/opaque channel recorded a fact per non-constant arg across the huge Go dependency closure; fixed by gating all function-value recording on a precomputed `indirectCallees` set (a function that never dispatches through a parameter can never consult the facts), which restored GoWithDeps to baseline (20.0M allocs, 1.66 GB). Corpus FP=0/recall=1.000 (222 samples), full suite green. |
| ENG-14 | med | 📋 | **A map lookup keyed by a tainted value taints the value it returns, and the resulting summary reaches sinks the source cannot call.** Found by dogfooding: `godzilla scan internal` on the `godzilla-playground` PR reported 11 findings (5 CWE-22, 6 CWE-79), all rooted in one source — an HTTP query parameter used ONLY as a key into a prebuilt `map[string]*fileEntry`. A key cannot select a value the map does not already hold, so the lookup launders it; the engine models it as propagating and the retrieved struct, its fields, and everything downstream come back tainted. Verified unexploitable: six traversal payloads (`../`, `%2e%2e%2f`, `....//`, absolute, and a `main.go/../` prefix) all 404. Two distinct problems sit behind the one symptom. (a) The container model has no case for INDEX-by-key on a map whose CONTENTS are untainted — `taintContainer`'s whole-container fallback is right for a store and wrong for a lookup, and a fix has to keep `params[key]` working, which is how Python/Ruby request sources are lowered, so this is not simply "stop propagating through map reads". (b) Worse, 10 of the 11 sinks sit in packages the source **cannot reach**: `internal/llm` and `internal/ruletest` are not in `go list -deps ./internal/playground`. Both they and the source merely share `internal/srclines` and `os.ReadFile`, so a context-insensitive parameter summary on the shared callee is being attributed back to unrelated callers. (b) is the more serious of the two — it makes one FP at a shared helper multiply across every caller of that helper — and it is worth reproducing on a small twin before touching the container model. |
...
| COV-11 | high | 🟡 | **Framework handler-parameter sources** (branch `claude/realworld-recall`). Shipped: Go free-function accessors (`go:*web.Params`); Python FastAPI/Tornado/MethodView handler-param synthesis (`py:@http.param`); `with open(...)` context-manager lowering; split/join propagators. Corpus TP 133→142, FP=0. Method-propagator **chaining** now forwards taint through a call-result receiver (`p.strip().split()`, `p.encode().decode()`): the frontend captures a `Call`/`Subscript` receiver so the propagator sees it (the `py:<dynamic>.<method>` callee already matched `py:*.<method>`). **JS handler-param synthesis** shipped: a route handler registered with a string path (`app.get("/x", (rq,res)=>…)`, any router verb) has its first parameter treated as the request object regardless of name — property reads off it canonicalize to `req` so the request-source globs match even when the param is not the conventional `req`/`request`/`ctx`. **Signature-destructured request params** now covered too: a handler that destructures its request object in the signature (`({ query, body: b }, res) => …`) binds each destructured property to a synthetic `js:req.<key>` source read, so the local (`query`, `b`) seeds request taint exactly as an in-body `req.query` access would. **Higher-order-callback taint and thread/async dispatch** now covered too (see ENG-13). **Django/DRF class-based-view handler params** now covered: the Python route-handler recognition's `handlerBaseClasses` table (the documented data-only extension point) gained the Django CBV + DRF `APIView` bases (`View`/`ListView`/`DetailView`/`TemplateView`/…/`APIView`/`GenericAPIView`), so a verb method's URL captures (`get(self, request, pk)` → `pk`) are seeded as `py:@http.param`. The `request` object param is **excluded** (the precise `request.GET`/`.POST`/`.data` accessor globs stay authoritative — whole-object request taint would be broader and less precise), so it adds recall with FP=0. Samples: `test/python/django_cbv{,_safe}`. **Open (documented follow-on):** DRF `ViewSet` action verbs (`list`/`retrieve`/`create`/`update`/`destroy`) are not yet in `handlerMethodVerbs`; Falcon `on_<verb>` resources need a distinct recognizer. |
...
| COV-13 | med | 🟡 | **Framework-abstracted sinks + library sources** — shipped FastAPI/Starlette `FileResponse` path-traversal sink (+ narrowed py-xss `*Response` to fix the resulting FP). **JS ORM raw-query SQLi sinks** now shipped (`js-sqli`): knex `raw`/`whereRaw`/`andWhereRaw`/`orWhereRaw`/`havingRaw`/`orderByRaw`/`groupByRaw`/`joinRaw`/`fromRaw` and Prisma `$queryRawUnsafe`/`$executeRawUnsafe`, each `#0`-pinned so a parameterized `knex.raw("… ?", [id])` (bind values at arg 1+) is not flagged; the safe tagged-template `$queryRaw` form is excluded. Samples: `test/js/sqli_knex_raw{,_safe}`, `test/js/sqli_prisma_raw`. **Open:** `express.static` (traversal already covered by generic `*.sendFile`/`*.createReadStream` sinks, so low marginal value), Jinja→SQL propagator; opt-in "exported-API param = untrusted" library-scan mode (systeminformation CVE-2021-21315). |
...
| CI-2 | crit | ✅ `b9f3df7` | Stable, line-independent finding fingerprints (`analysis.Fingerprint`) → baseline diff-aware gating: `-write-baseline` snapshots current findings, `-baseline` suppresses them so the gate fires only on NEW findings. |
...
| TRUST-11 | med | ✅ | **Real-world CVE benchmark harness** — **REMOVED; superseded by the `cve-recall` project skill.** `test/cvebench` was an opt-in (`GODZILLA_CVE_BENCH=1`) manifest of 7 pinned CVEs. It was deleted because it never asserted anything: the test only logged HIT/MISS and a ratio, so recall could fall to zero and it would still pass, and nothing in CI ran it. The skill covers the same ground far better — ~30 CVEs fetched live from OSV.dev, every language, fix-commit AND tag-diff ground truth, plus an HTML report. What is knowingly given up is a DETERMINISTIC recall floor: the skill's CVE set changes each run by design, so it measures breadth but cannot prove a regression. Real-world regression protection would need a re-pinned, asserting gate; the synthetic corpus (`test/corpus`, FP=0/FN=0, which does assert) remains the automated guard. Historical result from the removed harness: Manifest refreshed to 7 recent (2024-2025) CVEs across Python/Ruby/JS (SSRF/SQLi/code-inj/cmd-inj/path-traversal); recall **5/7** after gap-closing work — mlflow (request-attribute alias `a = request.args; a.get()` → source + `requests.request` sink), langflow (FastAPI body source + ast propagators), motioneye (Tornado source + cross-module subprocess flow), decidim (Ruby module/class-method lowering + `Arel.sql` sink), webpack (JS cross-module default-export resolution + memoize-wrapper alias). The 2 remaining needed **core engine capabilities** now delivered in ENG-13 (higher-order-callback taint + thread/async dispatch): **gradio** — higher-order callback taint (tainted data handed to a generic traversal helper that invokes a passed-by-reference callback); **pyload** — non-`self` method-call CHA (ENG-11), cross-method instance-field heap persistence (ENG-12), and now thread/async dispatch (ENG-13) are all in place. The gradio flip is gated on the callback-multiplicity of its traversal helper (the singleton-gate discipline — see ENG-13); pyload's remaining blocker is thread-object field aliasing across `run()`. Sinks/sources already modeled for both. |
