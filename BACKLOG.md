...
### FE-2 [CRITICAL] ✅ DONE (Python `447b6dc`, JS `f45b213`) (verified: PARTIAL) No import/require alias resolution in Python and JavaScript — aliased or destructured imports of sink modules are silent false negatives
...
- **DONE (both languages):** *Python* — `pyast.py` emits Import/ImportFrom names+asnames; `collectImportAliases` builds a module alias table (`import x as y`→`y→x`, `from m import n`→`n→m.n`, relative imports skipped) and `resolveDotted` rewrites a callee's root before the `py:` prefix. *JavaScript* — `collectRequireAliases` builds the table from top-level require bindings (`const cp = require('child_process')`, destructured `const { exec } = …`, and `require('m').member`) and `resolveRequire` rewrites the callee root. **Relative requires (`./db`) are deliberately not aliased** so cross-file resolution (`db.run`→`js:db.run`) is preserved — a corpus regression caught and pinned this. Existing alias-robust globs match the resolved (canonical) names, so no rule edits and the corpus stays green. Tested by `TestResolveDotted`/`TestCollectImportAliases` (py) + `command_injection_aliased` sample, and `TestCollectRequireAliases` (js) + `command_injection_require` sample; the aliased/destructured/`require().member` sinks that were false negatives now fire in both languages.
...
### FE-3 [CRITICAL] ✅ DONE (`6fb8ad5`) (verified: CONFIRMED) Rust Cargo support only builds library targets — plain binary crates and workspaces fail entirely
...
### COV-7 [MEDIUM] ✅ DONE (`dcfda8d`) Rust coverage misses axum (the dominant framework) sources and has no XSS/open-redirect packs
...
### LLM-9 [LOW] ✅ DONE (`0302ec7`) Hardwired single-provider reviewer: no local/offline or alternate-provider path despite a pluggable interface
...
- **DONE:** New `OpenAIReviewer` (`internal/llm/openai.go`) speaks any OpenAI-compatible `/chat/completions` endpoint over plain `net/http` (no SDK dependency), reusing the shared prompt + `parseVerdict`; it is configured by `GODZILLA_LLM_BASE_URL`/`OPENAI_BASE_URL` (e.g. `http://localhost:11434/v1` for Ollama), `OPENAI_API_KEY`, and `GODZILLA_LLM_MODEL`. A `NewReviewer(tb)` factory routes on `GODZILLA_LLM_PROVIDER` (`openai` → local/OpenAI one-shot; default → Anthropic with agentic tools), wired into the CLI and documented in `-llm-review` help. Anthropic's `ANTHROPIC_BASE_URL` (already honored by the SDK for proxies) is now documented too. Tested hermetically against an httptest mock: request shape (model + user prompt), verdict parsing, error-status fail-open, and factory provider selection.
