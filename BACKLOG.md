...
| TRUST-11 | med | ✅ | **Real-world CVE benchmark harness** — `test/cvebench` (opt-in `GODZILLA_CVE_BENCH=1`): a fix-diff-verified CVE manifest + a scan/score test reporting recall alongside the corpus F1. Regression guard for the COV-11/13 breadth gaps. Manifest refreshed to 7 recent (2024-2025) CVEs across Python/Ruby/JS (SSRF/SQLi/code-inj/cmd-inj/path-traversal); current recall **0/7**, each a correctly-pinned flow blocked by a per-entry modeling gap (unmodeled framework source, unmodeled sink, deep inter-file hop, or SSRF fixed-host suppression). |
