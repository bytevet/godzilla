...
| FE-12 | high | ✅ `9357b76` + follow-up | Flow-typed `.js` recovered by blanking Flow syntax in place (`flowstrip.go`), offset-preserving because `go-sourcemap` is consumer-only and positions are mandatory. parse-server now converts **196/196** (was 187/196 at the same checkout). The residual was not only casts: `export`/`declare` modifiers stranded by the blank, multi-line unions cut at their first newline, casts nested in a call or applied to a pattern, and Flow's generic bound `<T: X>`. Two silent-corruption bugs surfaced with them — a `{}` in a ternary branch reset the pending-`?` counter, and `type` used as an ORDINARY identifier (`(type as X)`, pervasive in React) read as a type alias and blanked the rest of the function while staying brace-balanced. Verified on parse-server (196 files) and React (1,873): zero blanked spans contain executable tokens. `hermes-parser` via a pure-Go WASM runtime remains the full-fidelity option if the subset ever proves too narrow; React still drops 238 files it does not cover. |
