...
### CI-5 [HIGH] ✅ DONE (`<pending>`) No config file and no path include/exclude filters — everything is CLI flags and hardcoded skips
...
### CI-8 [MEDIUM] ✅ DONE (`<pending>`) No version anywhere: no version subcommand, no tool version in SARIF or JSON, unversioned JSON schema
