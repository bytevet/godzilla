...
### FE-9 [MEDIUM] ✅ DONE (`<pending>`) Java frontend hard-requires JDK 24+ and hides the diagnostic when the helper fails
...
### FE-10 [MEDIUM] ✅ DONE (`<pending>`) Rust MIR lowering is regex-scraping an explicitly unstable text format with no version guard — a rustc upgrade silently zeroes findings
