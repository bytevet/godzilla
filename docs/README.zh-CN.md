...
Godzilla 把多种语言统一**下降**（lowering）为一套语言无关的 SSA 中间表示 **gIR**，再由
同一个跨过程污点分析引擎处理。**检测规则只需写一次，即可覆盖所有受支持的语言。**
...
- **YAML 规则，可精确到汇点参数。** 污点源（source）、汇点（sink）、净化函数（sanitizer）、
  传播函数（propagator）一律以规范名（canonical name）的通配符书写。汇点还能指定注入点参数
...
Go 前端会把依赖的**函数体**一并下降为 gIR，因此污点能够穿过库代码，而不是在库调用处中
断 —— 但大型仓库的传递依赖闭包动辄数百万行，把它们全部加载进来，可能在分析真正开始之
前就耗尽机器内存。`-dep-budget` 用来限定有多少第三方 Go 源码会被提升为完整分析：
...
> **macOS 上的行为变更。** `auto` 依据探测到的内存推算上限，而此前这套探测只支持 Linux
> —— 于是在 macOS 上 `auto` 实际等同于 `off`，整个依赖闭包都会被加载。现在它在 macOS 上
> 也会算出真实上限：默认扫描更快、更省内存，但也可能报出以前不会出现的 `DEGRADED`。
> **限额扫描与不限额扫描的检出集合可能不同** —— 限额改变了哪些依赖函数体会传递污点，
> 某条检出被归因到的来源也可能随之移动。如果你维护着 triage 基线，升级后请重新生成一次：
> 来源变了指纹就会变，否则原本已被抑制的检出会重新冒出来。传 `-dep-budget off` 可以完全
> 恢复旧行为。
...
可执行文件：它把目标下降一次，随后启动一个本地 Web 界面，用于查看 gIR 并在其上调试规
则；扫描流水线本身不受影响。
...
它只监听本地回环地址，且每次启动只下降一次：不监听文件变化，也不会重新下降。`make build`
...
仍按近似处理。SSRF 的降噪只在一种情况下抑制检出项：主机名**已被证明**是固定的，污点只落在
URL 的 path 或 query 上。因此它不会牺牲真实检出。
...
（通常只需写 YAML，参见[指南](writing-rules.md)）、新增一个语言前端，或让现有前端的下降
更完整。
