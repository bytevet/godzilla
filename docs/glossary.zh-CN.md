...
README 的章节名一旦定下就不再改动：标题变了，指向它的锚点也就断了。

| English | 中文 | 说明 |
|---|---|---|
| Try it | 快速试用 | 面向新用户的试跑章节。不用「试用一下」「快速开始」。 |
| What you get | 你会得到什么 | 「快速试用」下的小节，讲报告长什么样、退出码怎么读。 |
| Usage | 用法 | 常用命令与 flag。原先叫「快速上手」，该名字已随试跑章节的引入退休。 |
| Reference | 参考 | 进度显示、`-dep-budget`、Playground、环境变量的归档处。 |
