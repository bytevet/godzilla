...
Every change to a SAST engine must answer four questions before it merges:
...
The gate modifies nothing in the engine, gIR, rules, or tests — it's built
entirely from existing tooling (`git diff`, the corpus scorer
`TestCorpusSignalToNoise`, the rulepack YAML, and the Go benchmarks).
...
# Fast smoke run (fewer benchmark samples), report only, no gating:
scripts/pr-quality-gate.sh origin/main --bench-count 6 --no-gate

# Just the cheap metrics (skip the heavy benchmark + corpus steps):
scripts/pr-quality-gate.sh origin/main --no-bench --no-corpus
```
...
- `benchstat` for the benchmark comparison
  (`go install golang.org/x/perf/cmd/benchstat@latest`). If absent, the
  performance section is skipped.
...
- **Per-language full-pipeline scans**: `BenchmarkScan_GoWithDeps`, `BenchmarkScan_GoSimple`,
  ...
  **including the subprocess frontends** (python3/rustc/java/ruby) — so a per-language
  ...
  a benchmark whose toolchain is absent is skipped and doesn't gate.

`benchstat` compares `-count` samples and marks a change `~` when it is not
...
noisier than the pure-Go engine benchmarks — enough that at the usual `alpha=0.05`
a borderline noise result can read as significant. So the gate uses a strict
**`alpha=0.01`** (`--alpha`): only a strongly-supported regression trips it, which
suppresses run-to-run subprocess/GC noise while still catching a real slowdown on
the stable benchmarks. `--bench-count` and the thresholds are also tunable.
...
succeeds** — not on every push — because building and benchmarking both revisions
is expensive. Running only on PRs that already build and pass tests keeps the cost
down, and the `workflow_run` context has a write token, so the report is posted as
a single sticky PR comment (updated in place on each run).
