...
findings:                 # rules that MUST fire; empty list => sample must be CLEAN
  - rule: go-sql-injection
    ...
    max: 2                # optional: at most this many — what pins a *_safe half
    line: 62              # optional: a finding of this rule must land at this sink line
    sink: Query           # optional: a finding's sink callee must contain this substring
    path: [18, 20, 22]    # optional: source lines the taint path must visit, in order
```
...
2. **no other rule fires** — a false-positive guard (e.g. `complex_logic` declares
   `findings: []`, so any finding on that benign code fails the build), **and**
3. if `line`/`sink` are given, at least one finding of that rule matches that
   location — an independent, hand-written ground truth that upgrades the oracle
   from "rule fired" to "rule fired at the right place" (the count-only oracle is
   self-generated by `RegenerateManifests`, so it can't catch a wrong-location
   finding; `line`/`sink` are added by hand and not overwritten by regeneration),
   **and**
4. if `path` is given, one finding of that rule visits those source lines **in
   order** along its taint path. This is the same idea applied to the FLOW rather
   than its endpoint, and it catches what `line`/`sink` structurally cannot: a
   path that lost its middle still reports the right source and the right sink.
   Listed lines need not be adjacent, so adding an intermediate hop does not break
   an expectation — losing one does. Hand-written; regeneration preserves it.
