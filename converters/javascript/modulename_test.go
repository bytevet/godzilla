...
func TestModuleNameForUniqueness(t *testing.T) {
	...
	a := filepath.FromSlash("/proj/ssrf/app.js")
	b := filepath.FromSlash("/proj/xss/app.js")

	if got := moduleNameFor(root, a); got != "ssrf/app" {
		t.Errorf("moduleNameFor(root, ssrf/app.js) = %q, want %q", got, "ssrf/app")
	}
	if got := moduleNameFor(root, b); got != "xss/app" {
		t.Errorf("moduleNameFor(root, xss/app.js) = %q, want %q", got, "xss/app")
	}
