...
# Ruby (no gems), so the frontend needs only `ruby` on PATH.
#
# Ripper.sexp returns nested arrays whose head is a symbol tag (e.g. :program,
# :def, :call, :@ident); leaf tokens carry a [line, col] position. Symbols are
# serialized as plain strings so the Go side sees a regular JSON tree.
require 'ripper'
require 'json'
...
# Batch mode: one JSON document per line, in argument order, always exit 0 — a
# per-file failure is that file's own {"error": ...} line, and interpreter
# startup is paid once per batch instead of once per file. Single-file mode
# prints one bare JSON document with no newline.
if ARGV[0] == "--batch"
